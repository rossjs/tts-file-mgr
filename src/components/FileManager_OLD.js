  ...
  return (
    <div className={classes.root}>
      <DragDropContext onDragEnd={handleOnDragEnd}>
        <Droppable droppableId="mod" isCombineEnabled>
          {(provided) => (
            <Grid container {...provided.droppableProps} ref={provided.innerRef}>
              {folders.map((folder, index) => (
                <Draggable key={folder} draggableId={folder} index={index}>
                  {(provided) => (
                    <Droppable droppableId="folder" isCombineEnabled>
                      {(provided) => (
                        <Grid item key={folder} className={classes.item} xs={6} sm={2} md={2} {...provided.droppableProps} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
